...
class MockAgent:
    def __init__(self, url="ws://test", logger=None, name="MockAgent", hang_duration=10.0):
        self.url = url
    ...
    async def start(self):
        self.logger.info(f"Agent '{self.name}': start called.")
        self.running = True
...
async def test_game_times_out_and_stops_agents(base_config, caplog):
    ...
    agent1 = MockAgent(name="Agent1_Timeout", hang_duration=5)
    agent2 = MockAgent(name="Agent2_Timeout", hang_duration=5)
...
async def test_game_finishes_before_timeout(base_config, caplog):
    ...
    agent1 = MockAgent(name="agent1", hang_duration=0.1)
    agent2 = MockAgent(name="agent2", hang_duration=0.1)
...
async def test_game_timeout_disabled_with_zero_duration(base_config, caplog):
    ...
    agent1 = MockAgent(name="Agent1_NoTimeout", hang_duration=0.1)
...
async def test_game_timeout_disabled_with_negative_duration(base_config, caplog):
    ...
    agent1 = MockAgent(name="Agent1_NegTimeout", hang_duration=0.1)
    runner = GameRunner(config=base_config, agents=[agent1])
...
async def test_agent_start_raises_exception(base_config, caplog):
    ...
    failing_agent = MockAgent(name="FailingAgent", hang_duration=0.1)
